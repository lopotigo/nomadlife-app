...
- **Local Marketplace (Mercato Locale):** GPS-based peer-to-peer marketplace for nearby items. CRUD with proximity filter (1-5km radius), category filters, image upload. Routes under `/api/local-listings/*`. Schema: `local_listings` table. Page: `/local-marketplace`.
- **City Guides (Guide Città):** AI-generated city guides for major nomad hubs (Bangkok, Lisbon, Madeira, Bali, Chiang Mai). Categories: WiFi, coworking, visa, food, lifestyle. Map markers on unified map. Routes under `/api/city-guides/*`. Schema: `city_guides` table. Page: `/city-guides`.
